...
def ssim(fi_image: numpy.ndarray, se_image: numpy.ndarray)-> float:
    ...
    # covariance = numpy.cov(contrast_fi, contrast_se) # Ковариация двух случайных величин # Не работает
    const_fi = 0.0003
    const_se = 0.0005
    up =(2 * intensity_fi*intensity_se + const_fi) * ( 2 * contrast_fi * contrast_se  + const_se )
    down = (intensity_fi ** 2 + intensity_se ** 2 + const_fi) * ( contrast_fi ** 2 +  contrast_se ** 2 + const_se )
    return up/down
